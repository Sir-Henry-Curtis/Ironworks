...
import ironworks
from ironworks import RUNDIR, logger, DATA_DIR
...
def commitsBehind():
    ...
    url = 'https://api.github.com/repos/%s/ironworks/compare/%s...%s' % (user, ironworks.CURRENT_COMMIT, ironworks.LATEST_COMMIT)
    result = urllib2.urlopen(url).read()
    git = json.JSONDecoder().decode(result)
...
def checkGithub():
    ...
    try:
        ironworks.LATEST_COMMIT = latestCommit()
        if ironworks.FIRST_RUN:
            ironworks.CURRENT_COMMIT = ironworks.LATEST_COMMIT
            writeVersion(ironworks.CURRENT_COMMIT)
    except:
        logger.log('UPDATER :: Could not get latest commit from github', 'WARNING')

    if ironworks.CURRENT_COMMIT:

        try:
            ironworks.COMMITS_BEHIND = commitsBehind()
        except:
            logger.log('UPDATER :: Could not get commits behind from github', 'WARNING')

        if ironworks.COMMITS_BEHIND >= 1:
            logger.log('UPDATER :: Update available, you are %i commits behind' % ironworks.COMMITS_BEHIND, 'INFO')
            ironworks.COMMITS_COMPARE_URL = 'https://github.com/%s/ironworks/compare/%s...%s' % (user, ironworks.CURRENT_COMMIT, ironworks.LATEST_COMMIT)

        elif ironworks.COMMITS_BEHIND == 0:
            logger.log('UPDATER :: Up to date', 'INFO')

        elif ironworks.COMMITS_BEHIND == -1:
            logger.log('UPDATER :: Unknown version. Please run the updater', 'INFO')

    else:
        logger.log('UPDATER :: Unknown version. Please run the updater', 'INFO')

    return ironworks.COMMITS_BEHIND
...
def Update():
    ...
    try:
        ...
        writeVersion(ironworks.LATEST_COMMIT)
    except:
    ...
    root_src_dir = os.path.join(update_folder, '%s-ironworks-%s' % (user, ironworks.LATEST_COMMIT[:7]))
