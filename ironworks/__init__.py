...
def initialize():
    ...
    with INIT_LOCK:
        ...
        # Set up logger
        if not LOG_FILE:
            LOG_FILE = os.path.join(DATA_DIR, 'logs', 'ironworks.log')
        ...
        logger = ironworksLogger(LOG_FILE, VERBOSE)
        ...
        settings = Setting()
        get_setting_value = settings.get_setting_value

        if get_setting_value('ironworks_port'):
            port_arg = False
            for arg in ARGS:
                if arg == '--port' or arg == '-p':
                    port_arg = True
            if not port_arg:
                PORT = int(get_setting_value('ironworks_port'))

        # Set up AUTH
        username = get_setting_value('ironworks_username')
        password = get_setting_value('ironworks_password')
        ...
        if '--webroot' not in str(ARGS):
            WEBROOT = get_setting_value('ironworks_webroot')
...
def start():
    ...
    if __INITIALIZED__:
        ...
        if not DEVELOPMENT:
            try:
                logger.log('Starting IRONWORKS on %s:%i%s' % (HOST, PORT, WEBROOT), 'INFO')
                SERVER.start()
                while not True:
                    pass
            ...
            logger.log('Starting IRONWORKS development server on port: %i' % (PORT), 'INFO')
...
def stop():
    """Shutdown Ironworks"""
    logger.log('Shutting down IRONWORKS...', 'INFO')
...
def restart():
    ...
    popen_list += ARGS
    logger.log('Restarting IRONWORKS with: %s' % popen_list, 'INFO')
    SCHEDULE.shutdown(wait=False)
