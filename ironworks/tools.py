...
import os
import ironworks
from ironworks import app, logger, preferences, mDb
from ironworks.models.setting import Setting
from ironworks.models.xbmcServer import XbmcServer
...
def check_auth(username, password):
    ...
    """
    return username == ironworks.AUTH['username'] and password == ironworks.AUTH['password']
...
def requires_auth(f):
    ...
    def decorated(*args, **kwargs):
        if ironworks.AUTH['username'] is not None and ironworks.AUTH['password'] is not None:
            creds = ironworks.AUTH
...
def setDb():
    ...
    dbPassword = prefs.getPassword()
    host = prefs.getHost()
    dbName = prefs.getDbName()
    g.db = mDb.Db(dbName, host, userName, dbPassword)
    global db
    db = mDb.Db(dbName, host, userName, dbPassword)
...
def setUser(username):
    ...
    if username is not None:
        result = db.select("users", where={"username": username})
        user = result[0]
        userDict = {}
...
def xbmc_image(url, label='default'):
    ...
    if url.startswith('special://'):  # eden
        return '%s/xhr/xbmc_image/%s/eden/?path=%s' % (ironworks.WEBROOT, label, url[len('special://'):])

    elif url.startswith('image://'):  # frodo
        url = url[len('image://'):]
        url = urllib.quote(url.encode('utf-8'), '')

        return '%s/xhr/xbmc_image/%s/frodo/?path=%s' % (ironworks.WEBROOT, label, url)
    else:
        return url
